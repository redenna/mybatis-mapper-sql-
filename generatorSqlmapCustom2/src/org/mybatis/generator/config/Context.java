...
import change.MyBaseMapperImpl;
...
public class Context extends PropertyHolder {
    ...
    public void generateFiles(ProgressCallback callback,
        ...
        if (introspectedTables != null) {
        	if (introspectedTables.size() > 0)
        	{//添加要生成的Mapper泛型基类信息
        		MyBaseMapperImpl introspectedTable = new MyBaseMapperImpl(this, warnings, callback);
    ...
    public IntrospectedTable getIntrospectedTableByType(String qualifiedJavaType)
    {
    	for (IntrospectedTable it : introspectedTables)
    	{
    		String type = it.getFullyQualifiedObj();
    		String voQualifiedName = it.getTableConfigurationProperty(PropertyRegistry.ANY_VO_CLASS);
    		if (null != voQualifiedName)
    		{
    			int lastIdx = voQualifiedName.lastIndexOf('.');
    			if (-1 != lastIdx)
    				voQualifiedName = voQualifiedName.substring(lastIdx+1, voQualifiedName.length());
    		}
    			
    		if (qualifiedJavaType.equals(type) || qualifiedJavaType.equals(voQualifiedName))
    		{
    			return it;
    		}
    	}
