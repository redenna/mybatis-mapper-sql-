...
public class JavaMapperGenerator extends AbstractJavaClientGenerator {
	public static final String BASE_MAPPER_CLASS_NAME = "BaseGenMapper";
	public static final String BASE_MAPPER_CLASS_FULL = "BaseGenMapper<V extends R,R,E,P>";
	public static final String BASE_MAPPER_VO_RECORD_TYPE = "V"; 
	public static final String BASE_MAPPER_RECORD_TYPE = "R"; 
	public static final String BASE_MAPPER_EXAMPLE_TYPE = "E";
	public static final String BASE_MAPPER_PRIMARY_TYPE = "P";
	// po对象类型  Example 主键类型
    /**
     * 
     */
	...
	public List<CompilationUnit> getCompilationUnits() {
		...
		if (null != introspectedTable.getFullyQualifiedTable())
			fullyQualifiedTableName = introspectedTable.getFullyQualifiedTable().toString();
		...
		final String mapperType = introspectedTable.getMyBatis3JavaMapperType();
		final boolean bBaseMapper = (mapperType != null && mapperType.endsWith(BASE_MAPPER_CLASS_FULL))
				&& BASE_MAPPER_RECORD_TYPE.equals(baseRecordType) && BASE_MAPPER_EXAMPLE_TYPE.equals(exampleType);
		...
		if (!bBaseMapper) {
			...
			String voRecordType = baseRecordType;
			String voQualifiedName = introspectedTable.getTableConfigurationProperty(PropertyRegistry.ANY_VO_CLASS);
			if (stringHasValue(voQualifiedName)) {
				voRecordType = voQualifiedName;
				FullyQualifiedJavaType fqjt2 = new FullyQualifiedJavaType(voRecordType);
				interfaze.addImportedType(fqjt2);
			}
			
			String SuperInterfaceName = BASE_MAPPER_CLASS_NAME+"<"+voRecordType+","+baseRecordType+","+exampleType;
			if (introspectedTable.getRules().generatePrimaryKeyClass())
			{
				FullyQualifiedJavaType primaryType = new FullyQualifiedJavaType(introspectedTable.getPrimaryKeyType());
				interfaze.addImportedType(primaryType);
				SuperInterfaceName += ","+primaryType.getShortName();
			}else{
				List<IntrospectedColumn> introspectedColumns = introspectedTable.getPrimaryKeyColumns();
				if (introspectedColumns.size() > 0)
				{
					FullyQualifiedJavaType primaryType = introspectedColumns.get(0).getFullyQualifiedJavaType();
					SuperInterfaceName += ","+primaryType.getShortName();
				}
			}
			...
			FullyQualifiedJavaType fqjt2 = new FullyQualifiedJavaType(SuperInterfaceName);
			interfaze.addSuperInterface(fqjt2);
			interfaze.addImportedType(fqjt2);
		}
		...
		if (bBaseMapper) 
		{
			...
			FullyQualifiedJavaType fqjt = new FullyQualifiedJavaType(exampleType);
			interfaze.addImportedType(fqjt);
			
			fqjt = new FullyQualifiedJavaType(baseRecordType);
			interfaze.addImportedType(fqjt);
		}
